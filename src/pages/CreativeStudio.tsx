...
const CreativeStudio = () => {
  const { isCreativeMode, setMode } = useTheme();
  
  // Force creative mode on this page
  useEffect(() => {
    const previousMode = localStorage.getItem('themeMode') || 'automation';
    
    // Set creative mode when entering this page
    setMode('creative');
    
    // Restore previous mode when leaving
    return () => {
      if (!previousMode.includes('creative')) {
        setMode(previousMode as 'automation' | 'creative' | 'academy');
      }
    };
  }, [setMode]);
  ...
  return (
    <div className="bg-gradient-to-b from-black to-[#0c0225] text-white overflow-hidden">
      {/* Hero Section */}
